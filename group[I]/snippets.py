...
def policy_evaluation(env, policy, gamma, theta, max_iterations):
    ...
    for i in range(max_iterations):
        ...
        for s in range(env.n_states):
            s_value = value[s]
            value[s] = sum(
                [env.p(next_state, s, policy[s]) * (env.r(next_state, s, policy[s]) + gamma * value[next_state]) for
                 next_state in
                 range(env.n_states)])
...
def policy_improvement(env, value, gamma, policy):
    ...
    for s in range(env.n_states):
        ...
        v = [
            sum([env.p(next_state, s, a) * (env.r(next_state, s, a) + gamma * value[next_state]) for next_state in
                 range(env.n_states)])
